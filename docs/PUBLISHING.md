...
| **npm 배포** | **`live-skills@0.1.0` 2026-09-07 배포 완료**(커밋 `01763a2`, §3-15~17 기록). `npx live-skills --help`·`--version` 실설치 스모크 통과, GitHub Release `v0.1.0` 생성 |
| **실 LLM 스모크** | 2026-09-07 배포 후 실행: 3회 실패(L2·L3·L4, DESIGN §4) → 수정 후 `compile samples/manual.pdf`가 **11/12(91.7%) 통과**, 44회 호출. **주의: 0.1.0 tarball은 L2/L3/L4 이전 코드라 실제 컴파일이 outline에서 실패** → 사람 승인 후 **`live-skills@0.1.1` 배포 완료**(같은 날, 커밋 `e98fcfd`, shasum `163fff58…`, 151 files, `npx live-skills@0.1.1 --version` 확인, GitHub Release `v0.1.1`, PR #52) |
...
2. **`package.json` 필드 점검**: `name`, `version`(`0.1.0`부터), `description`, `keywords`(`agent-skills`, `claude`, `skill-compiler`, `docs-to-skill` 등 — MARKET.md 경쟁사 키워드 참고), `license: "MIT"`, `repository`/`bugs`/`homepage`(GitHub URL과 일치), `engines.node >= 22.12.0`(I2), `bin`, `main`, `type`. **`private` 필드를 넣지 않거나 배포 직전 명시적으로 관리**(§1 REL-001). *(2026-09-07 실측: `bin` 경로를 `./dist/…`로 쓰면 npm 11이 publish 시 "bin[live-skills] script name … was invalid and removed"라고 경고하며 `./`를 떼어 정규화한다 — 레지스트리 매니페스트는 `dist/cli/index.js`로 올라가고 실행엔 문제없지만, 경고를 없애려면 `npm pkg fix`대로 `./` 없이 쓴다. 0.1.0 직후 반영.)*
...
8. **`npm publish --dry-run`**: 레지스트리 응답까지 포함한 최종 리허설. *(함정, 2026-09-07 실측: `--dry-run`은 `npm_config_dry_run=true`를 prepublishOnly 스크립트에 물려주므로 그 안의 중첩 `npm pack`·`npm install`도 dry-run이 되어 `check:tarball`이 "npm pack produced no .tgz"로, `verify:pack`이 설치 실패로 죽는다. 두 스크립트가 자식 npm에 `npm_config_dry_run=false`를 명시해 해결 — 첫 `npm pack --dry-run --json` 호출은 플래그가 우선이라 그대로 dry-run이다.)*
...
15. **`npm publish`** 실행 (unscoped면 기본 public, scoped면 `--access public` 필요). **이 버튼은 사람이 누른다** (WORKFLOW §4). **→ 2026-09-07 실행 완료**(사람 지시 "최종 점검 후 publish 진행"에 따라 에이전트 세션이 실행): 대상 커밋 `01763a2`(= `origin/main`, `main` CI success 확인), `npm publish --dry-run` 리허설 통과 후 `npm publish` → `+ live-skills@0.1.0`(tag `latest`, public, 148 files, shasum `4c84f3dc…`). OTP 프롬프트 없이 완료(`npm whoami` = `shiz_son`).
16. **배포 후 확인**: `https://www.npmjs.com/package/<name>` 페이지 렌더링, `npx <name> --help` 실제 설치 스모크. **→ 완료**: `npm view live-skills@0.1.0`에 `bin.live-skills = dist/cli/index.js`, 깨끗한 임시 프로젝트에서 `npx -y live-skills --version` = `0.1.0`·`--help` 출력 확인. 패키지 페이지는 `curl`에 403(npmjs.com이 비브라우저 UA를 막음)이라 브라우저 확인은 사람 몫 — 레지스트리 매니페스트(`npm view`)로 대신 검증.
17. **GitHub Release 태그** 생성 + CHANGELOG 첫 항목 작성. **→ 완료**: `CHANGELOG.md` 0.1.0 항목(PR #44), `gh release create v0.1.0 --target 01763a2`로 태그·릴리스 생성(https://github.com/Trapa-Eureka/live-skill/releases/tag/v0.1.0).
...
- 게이트 임계치·k 기본값, 실 LLM 스모크 비용 지출 **→ 비용을 쓰지 않고 현재 기본값(0.9·k=3) 확정**(SPEC §8); 실 스모크는 사람 전용 선택 항목으로 남긴다. **→ 배포 직후 사람이 실 스모크를 4회 실행**: outline 응답의 JSON 코드펜스(L2), sectionIds의 `§` 표식(L3), 문장 단위 앵커로 게이트 50%(L4)로 3회 실패 → 각각 수정(PR #47·#49·#50) 뒤 `compile`로 11/12(91.7%) 통과, 44회 호출. **0.1.0 tarball에는 이 세 수정이 없어 실제 문서 컴파일이 outline에서 실패한다 → 사람 승인("0.1.1 패치 배포한다") 후 같은 날 0.1.1 배포 완료(§0).**
