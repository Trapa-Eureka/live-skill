...
에이전트 스킬(Agent Skills 표준)이 지식 전달 포맷으로 자리 잡으면서 "문서→스킬 컴파일" 카테고리가 열렸고, 대표 주자 book-to-skill(Python, MIT)은 수만 스타로 시장을 증명했다. 그러나 현재 세대는 **정적 소스의 1회 컴파일**이다: 소스가 바뀌면 사람이 fold-in으로 다시 넣어야 하고, 산출된 스킬이 원문 질문에 실제로 옳게 답하는지 의미 수준의 검증층이 없다.

live-skill은 그 두 빈칸을 제품의 축으로 삼는 **별도 신규 제품**(참고는 하되 코드 무관, TS/npm)이다.

**방어력 순서 (합의 사항)**: 4(도메인 팩, 자매 레포) > 2(품질 게이트) > 1·3(워처·MCP — 기능이라 모방 가능, 선점 효과용). 따라서 v0.1은 2를 완성해 "신뢰할 수 있는 컴파일러"라는 정체성을 먼저 세운다.
...
- [ ] npm 패키지명: live-skill 가용성 조사 + 후보 2개 (T11)
