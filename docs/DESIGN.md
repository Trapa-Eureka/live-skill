...
**L2 결정(2026-09-07) — JSON 응답의 "봉투"는 관용, 값은 엄격**: 0.1.0 배포 직후 첫 실 LLM 스모크가 outline 단계에서 "응답이 스키마와 맞지 않는다"로 죽었다. outline·qaGen은 "코드펜스 없이 JSON만" 요청하지만 모델은 여전히 ```json 펜스나 앞뒤 한 문장을 붙이곤 하고, 예전 코드는 `JSON.parse(raw)`를 그대로 불러 그 봉투 때문에 실패했다(값 자체는 멀쩡했을 수 있다). `core/jsonResponse.ts`의 `parseJsonResponse`가 (1) 둘러싼 코드펜스를 벗기고, (2) 그래도 파싱이 안 되면 가장 바깥 `{…}`/`[…]` 구간만 취한 뒤, (3) 그 결과를 **기존 zod 스키마로 그대로 엄격 검사**한다 — 잘린 JSON을 고치거나 필드를 추측하지 않으므로 게이트·형식 경계(C1)는 완화되지 않는다. 함께 `outline_invalid` 메시지에 실패 이유(`not valid JSON (…)` 또는 `schema: 경로: 규칙`)를 실어 사람이 "재시도"와 "프롬프트/모델 문제"를 구분할 수 있게 했다 — 값은 echo하지 않고 경로·규칙만, `sanitizeExternalText`를 거친다. qaGen의 파싱 실패는 예전처럼 0문항(재생성 1회 → 미검증, B2)이다.

**L3 결정(2026-09-07) — outline의 sectionIds에서 `§` 표식은 벗긴다**: L2 다음 스모크에서 outline이 통과한 뒤 커버리지 검사(B1)가 "unknown section ids: §overview, …"로 실패했다. 프롬프트가 섹션을 `[§<id>] (level n) 제목` 헤더로 보여주므로 모델이 id를 `§overview`처럼 표식째 베껴 쓴 것이다. (1) outline system에 "sectionIds에는 § 뒤의 id만, § 기호·대괄호 없이"를 명시하고, (2) `normalizeSectionIdRef`(core/sectionId.ts)가 `§id`·`[§id]`를 `id`로 정규화한 뒤 커버리지 검사에 넘긴다. 섹션 id는 `slugifyHeading`이 만들어 `§`·대괄호·공백을 절대 포함하지 않으므로 이 사상은 일대일이고, 표식이 없는 값은 손대지 않는다(모르는 id는 여전히 모르는 id로 실패) — B1의 "정확히 한 번" 검사는 그대로다.

**L4 결정(2026-09-07) — 앵커는 "가장 짧은 사실 구간", 증류는 값·식별자를 원문 그대로**: L3 뒤 첫 완주 스모크(34회 호출)는 게이트 50%(6/12)로 실패했고 실패 6건 중 5건이 `anchor_missing`이었다 — 샘플은 섹션 4개·각 한 문단이라 줄바꿈 문제는 아니고, qaGen이 문장 단위 인용을 고른 반면 distill은 "구조 추출"로 문장을 다시 써서 T7-2의 **글자 그대로 포함** 검사에 걸린 것이다(installation 3/3 통과는 `500 mA`·`192.168.50.0/24` 같은 짧은 값이 인용된 덕). 규칙(T7-2)은 그대로 두고 프롬프트 양쪽을 맞췄다. (1) qaGen: 질문은 구체적 사실(값·한계·시간·주소·식별자·명령·부품·단계)에 대해서만, `anchorQuote`는 그 사실을 담은 **가장 짧은 연속 구간**(대개 2~8단어, 문장 전체 금지). (2) distill: 수치·단위·임계치·시간·주소·식별자·명령·부품명은 **원문 표기 그대로**("five seconds"를 "5 s"로 바꾸지 않는다), 게이트가 그 구간을 그대로 찾는다고 명시. **열린 결정(사람, WORKFLOW §4·가드레일 1)**: 그래도 서식 차이(대소문자·공백·마크다운 강조·따옴표)로 놓치는 경우가 남으면 T7-2를 "정규화 후 포함"으로 고칠지(B안, 단어는 여전히 동일해야 함), 또는 `anchor_missing`을 실패가 아닌 리포트 항목으로 내리고 grader의 (b) 앵커 무모순 판정에 맡길지(C안, 게이트 완화)를 결정해야 한다 — 코드는 결정 전까지 T7-2 그대로다. **결과(같은 날)**: 이 프롬프트로 다음 실행(`compile`, 44회 호출)에서 12문항 중 11개 통과(91.7%), `anchor_missing` 0건 — 앵커가 전부 `500 mA`·`five seconds`·`power, link, and fault`류 짧은 구간으로 바뀌었다. 남은 1건은 픽스처 면책 문구("used only as a sample for this project")의 "목적"을 묻는 질문에 기능 설명으로 답해 grader가 WRONG 처리한 정당한 오답. 서식 차이 미스는 관찰되지 않아 B/C안 결정은 보류한다.
